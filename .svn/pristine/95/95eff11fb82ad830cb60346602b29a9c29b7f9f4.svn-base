...
class QLearningAgent(ReinforcementAgent):
  ...
  def update(self, state, action, nextState, reward):
    ...
    sample = (self.discount * nextValue) + reward
    newQVal = ((1 - self.alpha) * currVal) + (self.alpha * sample)
...
class ApproximateQAgent(PacmanQAgent):
  ...
  def getQValue(self, state, action):
    ...
    for key in feat:
      qVal += feat[key] * self.weights[key]
    # print qVal
    return qVal
    # util.raiseNotDefined()
  ...
  def update(self, state, action, nextState, reward):
    ...
    correction = ((reward + (discount * nextVal)) - qVal)

    features = self.featExtractor.getFeatures(state, action)
    for key in features:
      self.weights[key] += alpha * correction * features[key]
